...
import { Issue, IssueStatus, IssuePriority, Category, User } from '../../types';
import {
  Shield,
  Layers,
  Tag,
...
export const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'issues' | 'categories' | 'users'>('issues');

  // Status Change Modal State
  const [selectedIssueForStatus, setSelectedIssueForStatus] = useState<Issue | null>(null);
  const [targetStatus, setTargetStatus] = useState<IssueStatus>('in_review');
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, status, note }: { id: string; status: IssueStatus; note?: string }) =>
      api.updateAdminStatus(id, status, note),
    onSuccess: () => {
  ...
  const priorityMutation = useMutation({
    mutationFn: ({ id, priority }: { id: string; priority: IssuePriority }) =>
      api.updateAdminPriority(id, priority),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-issues'] });
      queryClient.invalidateQueries({ queryKey: ['issues'] });
    },
  ...
  const toggleCategoryMutation = useMutation({
    mutationFn: ({ id, is_active }: { id: string; is_active: boolean }) =>
      api.updateAdminCategory(id, { is_active }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-categories'] });
      queryClient.invalidateQueries({ queryKey: ['categories'] });
    },
  ...
  // User role/status mutation
  const updateUserMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { role?: string; account_status?: string } }) =>
  ...
  return (
    <div className="app-container" style={{ padding: 'var(--space-6) var(--space-4)' }}>
      {/* Admin Title */}
      ...
      {activeTab === 'issues' && (
        <div>
          {isLoadingIssues ? (
            ...
            <EmptyState title="No Issues in Queue" description="All reported issues have been processed." />
          ) : (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 'var(--space-4)' }}>
            {categories.map((cat) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    toggleCategoryMutation.mutate({ id: cat.id, is_active: !cat.is_active })
                  }
          ...
          >
            ...
            <form onSubmit={handleSaveStatus} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
              ...
              {statusMutation.isError && (
                ...
                >
                  {(statusMutation.error as any)?.message || 'Status update failed'}
                </div>
              )}
