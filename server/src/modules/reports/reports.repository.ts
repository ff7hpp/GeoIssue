...
import { DbReport } from '../../shared/types.js';
...
export const reportsRepository = {
  ...
  async findByIssueId(issueId: string): Promise<DbReport[]> {
    if (isUsingMockDb) {
      const list = Array.from(mockStore.reports.values())
  ...
  async update(
    ...
    if (isUsingMockDb) {
      ...
      if (data.image_url !== undefined) report.image_url = data.image_url;
