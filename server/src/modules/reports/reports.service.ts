...
export const reportsService = {
  async createReport(
    ...
    if (candidates.length > 0) {
      // Attach to nearest active matching candidate
      const nearest = candidates[0];
      targetIssueId = nearest.id;
      distanceToIssue = nearest.distance;
      await issuesRepository.incrementReportCount(targetIssueId);
    } else {
      // Create a brand new canonical Issue
      isNewIssue = true;
      const title =
        data.title?.trim() ||
        `${category.name} report near [${data.latitude.toFixed(4)}, ${data.longitude.toFixed(4)}]`;
