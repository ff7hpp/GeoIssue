...
const commentsRepository = {
  async findByIssueId(issueId) {
    if (isUsingMockDb) {
      const list = Array.from(mockStore.issue_comments.values()).filter((c) => c.issue_id === issueId).map((c) => {
        const user = mockStore.users.get(c.user_id);
        return {
          ...c,
          user: user ? { id: user.id, display_name: user.display_name, role: user.role } : void 0
        };
      }).sort(
