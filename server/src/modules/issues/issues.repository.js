...
const issuesRepository = {
  ...
  async list(filters = {}) {
    ...
    if (isUsingMockDb) {
      ...
      if (filters.search) {
        ...
        all = all.filter(
          (i) => i.title.toLowerCase().includes(s) || i.summary && i.summary.toLowerCase().includes(s)
        );
      }
      all.sort(
  ...
  async getStatusHistory(issueId) {
    if (isUsingMockDb) {
      const list = Array.from(mockStore.issue_status_history.values()).filter((h) => h.issue_id === issueId).map((h) => {
        const user = h.changed_by_user_id ? mockStore.users.get(h.changed_by_user_id) : null;
        return {
          ...h,
          changed_by: user ? { id: user.id, display_name: user.display_name, role: user.role } : void 0
        };
      }).sort(
        (a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
      );
