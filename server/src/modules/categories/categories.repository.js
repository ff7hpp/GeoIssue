...
const categoriesRepository = {
  async listAll(onlyActive = true) {
    if (isUsingMockDb) {
      return Array.from(mockStore.categories.values()).filter((c) => onlyActive ? c.is_active : true).sort((a, b) => a.name.localeCompare(b.name));
    }
    const sql = onlyActive ? "SELECT * FROM categories WHERE is_active = true ORDER BY name ASC" : "SELECT * FROM categories ORDER BY name ASC";
