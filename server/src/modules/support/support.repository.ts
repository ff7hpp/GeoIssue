...
export const supportRepository = {
  async hasUserSupported(issueId: string, userId: string): Promise<boolean> {
    if (isUsingMockDb) {
  ...
  async addSupport(issueId: string, userId: string): Promise<boolean> {
    if (isUsingMockDb) {
  ...
  async removeSupport(issueId: string, userId: string): Promise<boolean> {
    if (isUsingMockDb) {
      const key = `${issueId}:${userId}`;
  ...
  async getSupportersCount(issueId: string): Promise<number> {
    if (isUsingMockDb) {
      let count = 0;
