...
// Opt-in, repeatable development fixtures. Never substitutes for a real DB.
...
const locations = ['Kızılay', 'Çankaya', 'Bahçelievler', 'Ulus', 'Dikmen', 'Emek'];
const problems = [
...
try {
  ...
  for (let n = 0; n < 36; n++) {
    ...
    const resident = n % 3;
    const user = await client.query(`INSERT INTO users (firebase_uid,email,display_name,role)
    ...
    const date = new Date(Date.now() - n * 86400000).toISOString();
    await client.query(`INSERT INTO issues (id,category_id,title,summary,latitude,longitude,status,created_at)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (id) DO NOTHING`,
    [id, category.rows[0].id, `${locations[Math.floor(n / 6)]}: ${title}`, description, lat, lon, statuses[Math.floor(n / 6)], date]);
    await client.query(`INSERT INTO reports (id,issue_id,user_id,category_id,description,latitude,longitude,created_at)
