...
import { initDb, mockStore, isUsingMockDb } from '../db/pool.js';
...
describe('Issue Comments System', () => {
  ...
  it('should validate and create a comment on an existing issue', async () => {
    // Check if issues exist or query first issue
    ...
    if (issues.length > 0) {
      ...
      // Verify fetching comments for that issue
  ...
  it('should reject deleting a comment by unauthorized user', async () => {
    ...
    if (issues.length > 0) {
      ...
      // Try deleting with another user ID
      await expect(
